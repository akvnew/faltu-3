...
OAUTH2_CLIENT_SECRET_FILE = os.path.join(__dir__, 'client_secret.json')
TOKEN_FILE = os.path.join(__dir__, 'token.json')

def get_credentials(token_file, scopes=None, oauth2_client_secret_file=None):
    creds = None
...
def main():
    gdrive_service = build('drive', 'v3', credentials=get_credentials(
        token_file=TOKEN_FILE,
        scopes=SCOPES,
        oauth2_client_secret_file=OAUTH2_CLIENT_SECRET_FILE
    ))

    # download_folder(gdrive_service, 'exampleFolderId', 'destinationFolder')
    upload_folder(gdrive_service, os.path.join(__dir__, 'folderToUpload'))
